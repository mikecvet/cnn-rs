...
impl Convolution {
  ...
  pub fn
  patches (&mut self, image: &Array2<f64>) -> Vec<Patch2>
  {
    let mut data: Vec<Patch2> = Vec::new();
  ...
  { 
    ...
    for p in ctx.patches.as_ref().unwrap() {
      for k in 0..self.num_kernels {
        output.slice_mut(s![k, .., ..]).scaled_add(input[[p.x, p.y, k]], &p.data);
      }
    }

    self.kernels = self.kernels.clone().sub(ctx.alpha * output.clone());
